...
interface PaymentCanceledEmailProps {
  orderUrl: string;
  orderNumber?: string;
  customerName: string;
...
export const PaymentCanceledEmail = ({
  orderUrl,
  orderNumber,
  customerName,
  restaurantName,
  ...
  supportEmail,
  logoUrl,
}: PaymentCanceledEmailProps) => {
  return React.createElement(
    Html,
    null,
    React.createElement(Head),
    React.createElement(Preview, null, `Payment cancelled for your ${companyName} order`),
    React.createElement(
      Body,
      { style: main },
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(Text, { style: eyebrow }, 'Payment update')
        ),
        React.createElement(
          Section,
          { style: content },
          React.createElement(Text, { style: warningPill }, 'Payment cancelled'),
          React.createElement(Text, { style: title }, 'Your payment was cancelled'),
          React.createElement(
            Text,
            { style: message },
            `Hi ${customerName}, your payment for ${totalAmount} was cancelled.${
              restaurantName ? ` This was for your order from ${restaurantName}.` : ''
            }`
          ),
          React.createElement(
            Section,
            ...
            React.createElement(
              ...
              { style: summaryLabel },
              'Status',
              React.createElement('span', { style: warningValue }, 'Cancelled')
            )
          ),
          React.createElement(
            Section,
            ...
            React.createElement(
              Text,
              { style: noteText },
              'If this was a mistake, open your order page and start a new payment if it is still available.'
            )
          ),
          React.createElement(
            Section,
...
const warningValue = { float: 'right' as const, color: '#8a5a00', fontWeight: 700 };
const note = {
  marginTop: '20px',
  padding: '16px',
  backgroundColor: '#fff9f0',
...
export default PaymentCanceledEmail;
